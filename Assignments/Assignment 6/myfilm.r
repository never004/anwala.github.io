...
df <- user[ which(user$age == 23 & user$gender == 'M' & user$occupation == 'student'), ]
...
get.ratings <- function(u.id, i.cors) {
  ...
  item.id <- append(item.id, unlist(lapply(unique(item.id), top.5.items, u.id=37, i.cors=item.cors)))
  #top movies
  topuser <- data[data$user.id == u.id,][c("item.id", "rating")]
  items <- topuser[order(topuser[,2], decreasing=TRUE),]
  my.top.10 <- head(items$item.id, 10)
  #similar top movies
  item.id <- append(item.id, unlist(lapply(my.top.10, top.5.items, u.id=37, i.cors=item.cors)))
...
get.hatings <- function(u.id, i.cors) {
  ...
  item.id <- append(item.id, unlist(lapply(my.bottom.10, top.5.items, u.id=37, i.cors=item.cors)))
  as.data.frame(table(item.id))
}
hatings <- get.hatings(37, item.cors)
#how about we combine everything?
rec <- merge(x=ratings, y=hatings, by.x='item.id', by.y='item.id', all=T)
rec[is.na(rec)] <- 0
...
#######################
#Home Alone is my favorite movie (ID 94)
#Robocop 3 is my least favorite movie (ID 1274)
#most correlated movies to Home Alone
...
#least correlated movies to Home Alone
mytopdataleast <- as.data.frame(item.cors[94,])
colnames(mytopdataleast) <- c('cor')
rows <- rownames(mytopdataleast)
head(mytopdataleast[order(mytopdataleast, decreasing=FALSE), , drop=FALSE])
#most correlated movies to Robocop 3
mybottomdatamost <- as.data.frame(item.cors[1274,])
colnames(mybottomdatamost) <- c('cor')
rows <- rownames(mybottomdatamost)
head(mybottomdatamost[order(mybottomdatamost, decreasing=TRUE), , drop=FALSE])
#least correlated movies to Robocop 3
mybottomdataleast <- as.data.frame(item.cors[1274,])
colnames(mybottomdataleast) <- c('cor')
rows <- rownames(mybottomdataleast)
head(mybottomdataleast[order(mybottomdataleast, decreasing=FALSE), , drop=FALSE])